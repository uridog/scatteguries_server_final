...
def handle_client(client_socket):
    ...
    client_login = Client_communication()
    """Handles a single client connection."""
    client_name = client_login.log_in_protocol(client_socket)
    while True:
        # ready message
        client_login.handle_msg(client_socket)
        for i in range(3):
            client_done = False
            finished_game_counter = 0
            broadcast_end = False
            ...
            for c in clientsReady:
                if c[0] == client_name:
                    c[1] = False
            data = client_login.handle_msg(client_socket)
                ...
                try:
                    if data == " ":
                        if (time.time() - start_time) > 5 and broadcast_end is False:
                            ...
                            client_socket.send(create_msg("", "12").encode())
                    else:
                        myText = data.lower()
                        myText = myText.title()
                        ...
                        if 0 not in categories_arr and broadcast_end is False:
                            ...
                            broadcast_end = True
                            client_done = True
                        elif (time.time() - start_time) > 30 and broadcast_end is False:
                            broadcast(msgToSend, "10", clients)
                            broadcast_end = True
                            client_done = True
                        else:
